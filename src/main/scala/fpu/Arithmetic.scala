...
class FPU(expWidth: Int, sigWidth: Int) extends Module {
  val io = IO(new Bundle {
    val a = Input(new Float(expWidth, sigWidth))
    val b = Input(new Float(expWidth, sigWidth))
    val out = Output(new Float(expWidth, sigWidth))
  })

  val floatMul = Arithmetic.FloatArithmetic.cast(io.a) * io.b
  io.out := floatMul
}

object FPU extends App {
  (new ChiselStage).execute(args, Seq(ChiselGeneratorAnnotation(() => new FPU(expWidth = 8, sigWidth = 23))))
}